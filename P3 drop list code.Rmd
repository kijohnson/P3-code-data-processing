...
- The difference between the absolute value of the current exposure survey percent and the mean exposure survey percent is < 0.75 the standard deviation of the mean AND 
...
final<- data_stats3  %>%
  ...
  mutate(abs_MVar_diff = abs(MVar_diff)) %>%
  mutate(three_fourth_sd = 0.75*sd) %>%
